...
# ----------------------------
# 🌌 Orion Chaos Operator ULTRA Monitoring
# ----------------------------
...
# ---- Création fallback config si absent ----
if [[ ! -f "$CFG_PATH" ]]; then
  echo -e "${YELLOW}⚠️ Config introuvable, création d'un fichier par défaut...${RESET}"
...
launch_worker() {
    ...
    if ! docker-compose -f "$DOCKER_COMPOSE_PATH" up -d --build; then
        echo -e "${RED}💥 Erreur lors du build/lancement du worker!${RESET}"
...
# ---- Lancer watch + keep-alive ----
live_watch &   # Watch en arrière-plan
keep_worker_alive
